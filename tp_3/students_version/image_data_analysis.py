...
def get_radial_PS(average_PS):
    ...
    average_PS_radial = [0]*radial_freq.shape[0]

    for i in range(len(radial_freq)) : 
    	average_PS_radial[i] = compute_mean_circle(average_PS, radial_freq[i])

    return radial_freq, np.array(average_PS_radial)


def decoupe_image(image, grid_size ) : 
	""" 
	Permet de découper une image selon les dimensions données
	"""
